...
def price_cmp(p1, p2, threshold):
    return abs(float(p1) - float(p2)) / abs(float(p1) + float(p2)) > threshold


def main(threshold, timestamp):
    ...
    with gzip.open(fn1, 'rt') as f1, gzip.open(fn2, 'rt') as f2, open(output, 'a+') as csvFile:
        ...
        try:
            while True:
                ...
                ask2 = row2['askp0']
                if ts1 == ts2:
                    if price_cmp(bid1, bid2, threshold) or price_cmp(ask1, ask2, threshold):
                    ...
                    if price_cmp(bid1, p_row2.get('bidp0', 0), threshold)\
                            or price_cmp(ask1, p_row2.get('askp0', 0), threshold):
                        csvwriter.writerow([row1[i] for i in inf] + [p_row2.get(i) for i in inf])
                    p_row1 = row1
                    row1 = reader1.__next__()
                elif p_ts1 < ts2 < ts1:
                    if price_cmp(bid2, p_row1.get('bidp0', 0), threshold)\
                            or price_cmp(ask2, p_row1.get('askp0', 0), threshold):
                        csvwriter.writerow([p_row1.get(i) for i in inf] + [row2[i] for i in inf])
                    p_row2 = row2
                    row2 = reader2.__next__()
...
if __name__ == '__main__':
    main(threshold=0.015, timestamp='timestamp')
